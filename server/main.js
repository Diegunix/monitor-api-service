...
Meteor.startup(() => {
  ...
  process.env.MAIL_URL = "smtp://user:password@smtp.sendgrid.net:587"; //only change 'yourApiKey' and the port(if required)
...
Meteor.methods({
  ...
  "sendErrorEmail": function (api, userEmail) {
    ...
    Email.send({
      to: userEmail,
      from: "monitorservice@mail.com",
      subject: "Monitor API Service - API Error",
